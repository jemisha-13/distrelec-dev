...
describe('parseTraceparent', () => {
  ...
  it('should throw an error if traceparent is not 55 characters long', () => {
    ...
    expect(() => parseTraceparent(shorterString)).toThrow(InvalidTraceparentLengthError);
    expect(() => parseTraceparent(longerString)).toThrow(InvalidTraceparentLengthError);
  });
  ...
  it('should throw an error if traceparent does not match the W3C pattern', () => {
    ...
    unsupportedFormats.forEach((traceparent) => {
      expect(() => parseTraceparent(traceparent)).toThrow(InvalidTraceparentFormatError);
    });
  });

  // TODO: Enable if we switch to Jest
