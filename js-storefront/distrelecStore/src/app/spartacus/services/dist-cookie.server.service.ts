...
export class DistServerCookieService extends DistCookieService {
  ...
  check(name: string): boolean {
    ...
    const regExp: RegExp = DistServerCookieService.getCookieRegExp(name);
  ...
  get(name: string): string {
    if (this.check(name)) {
      ...
      return result[1] ? DistServerCookieService.safeDecodeURIComponent(result[1]) : '';
    } else {
      return '';
    }
  }
  ...
  getAll(): { [key: string]: string } {
    ...
    if (cookieString && cookieString !== '') {
      cookieString.split(';').forEach((currentCookie) => {
        ...
        cookies[DistServerCookieService.safeDecodeURIComponent(cookieName.replace(/^ /, ''))] =
          DistServerCookieService.safeDecodeURIComponent(cookieValue);
      });
    }
