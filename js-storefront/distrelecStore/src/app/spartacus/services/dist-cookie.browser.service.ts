...
export class DistBrowserCookieService extends DistCookieService {
  ...
  check(name: string): boolean {
    ...
    const regExp: RegExp = DistBrowserCookieService.getCookieRegExp(name);
  ...
  get(name: string): string {
    if (this.check(name)) {
      ...
      return result[1] ? DistBrowserCookieService.safeDecodeURIComponent(result[1]) : '';
    } else {
      return '';
    }
  }
  ...
  getAll(): { [key: string]: string } {
    ...
    if (cookieString && cookieString !== '') {
      cookieString.split(';').forEach((currentCookie) => {
        ...
        cookies[DistBrowserCookieService.safeDecodeURIComponent(cookieName.replace(/^ /, ''))] =
          DistBrowserCookieService.safeDecodeURIComponent(cookieValue);
      });
    }
