 ...
 *
 */
package com.namics.distrelec.b2b.storefront.interceptors.beforecontroller;
...
public class RequireHardLoginBeforeControllerHandler implements BeforeControllerHandler {
    ...
    public boolean beforeController(final HttpServletRequest request, final HttpServletResponse response, final HandlerMethod handler) throws Exception {
        ...
        if (request.isSecure()) {
            ...
            if (annotation != null) {
                ...
                if (((!anonymousUser || Boolean.TRUE.equals(getSessionService().getAttribute(WebConstants.ANONYMOUS_CHECKOUT))) && checkForGUIDCookie(request,
                        response, guid))) {
                    redirect = false;
                }
    ...
    protected boolean checkForGUIDCookie(final HttpServletRequest request, final HttpServletResponse response, final String guid) {
        if (guid != null && request.getCookies() != null) {
            final String guidCookieName = getGuidCookieGenerator().getCookieName();
                ...
                for (final Cookie cookie : request.getCookies()) {
                    if (guidCookieName.equals(cookie.getName())) {
                        if (guid.equals(cookie.getValue())) {
                            ...
                            getGuidCookieGenerator().removeCookie(response);
                        }
                    }
                }
            }
    ...
    protected CookieGenerator getGuidCookieGenerator() {
        return guidCookieGenerator;
    }

    public void setGuidCookieGenerator(final CookieGenerator guidCookieGenerator) {
        this.guidCookieGenerator = guidCookieGenerator;
    }
