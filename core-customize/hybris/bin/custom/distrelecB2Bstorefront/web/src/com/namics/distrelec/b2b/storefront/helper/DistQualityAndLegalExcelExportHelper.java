package com.namics.distrelec.b2b.storefront.helper;

import static com.namics.distrelec.b2b.storefront.util.DistCustomerAddressUtil.getCustomerAddress;
import static com.namics.distrelec.b2b.storefront.util.ExcelImageUtil.EXPAND_COLUMN;
import static com.namics.distrelec.b2b.storefront.util.ExcelImageUtil.addImageToSheet;
...
import com.namics.distrelec.b2b.storefront.util.ExcelImageUtil;
...
@Component
public class DistQualityAndLegalExcelExportHelper {
    ...
    public XSSFWorkbook generateEnvironmentalInformationXlsReport(List<ProductData> products) throws IOException {
        ...
        // Customer Details
        rowId.increment();
        defineCustomerDetails(workbook, sheet, rowId);

        // Article Details
        rowId.increment();
        defineArticleDetails(workbook, sheet, rowId, products);
    ...
    private void defineHeader(XSSFWorkbook workbook, Sheet sheet, MutableInt rowId) throws IOException {
        ...
        CellRangeAddress region = CellRangeAddress.valueOf("B" + excelRow + ":K" + excelRow);
    ...
    private void defineCustomerDetails(XSSFWorkbook workbook, Sheet sheet, MutableInt rowId) {
        ...
        cell.setCellValue(currentCustomer.getFirstName() + " " + currentCustomer.getLastName());
        ...
        region = CellRangeAddress.valueOf("G" + excelRowFrom + ":K" + excelRowTo);
        sheet.addMergedRegion(region);
        addTopBordersForRegion(sheet, region);

        // Thick black border at the end of Customer details (and above Article details)
        region = CellRangeAddress.valueOf("B" + excelRowFrom + ":K" + excelRowTo);
        addBottomBordersForRegion(sheet, THICK, region);
    }
    ...
    private void defineArticleDetails(XSSFWorkbook workbook, Sheet sheet, MutableInt rowId, List<ProductData> products) {
        ...
        excelRow = rowId.intValue() + 1; // because rowId starts the count from 0, but Excel counts rows from 1
        region = CellRangeAddress.valueOf("B" + excelRow + ":K" + excelRow);
        addBottomBordersForRegion(sheet, THICK, region);
    }
    ...
    private void defineDisclaimerText(XSSFWorkbook workbook, Sheet sheet, MutableInt rowId) {
        ...
        int excelRow = rowId.intValue() + 1; // because rowId starts the count from 0, but Excel counts rows from 1
        sheet.addMergedRegion(CellRangeAddress.valueOf("B" + excelRow + ":K" + excelRow));
    }
    ...
    private void defineReportDate(XSSFWorkbook workbook, Sheet sheet, MutableInt rowId) {
        ...
        sheet.addMergedRegion(CellRangeAddress.valueOf("C" + excelRowFrom + ":K" + excelRowTo));
        rowId.add(2);
    }
    ...
    private void addDistrelecLogo(Sheet sheet) throws IOException {
        ...
        ExcelImageUtil.ImageCreationDetails imageCreationDetails = new ExcelImageUtil.ImageCreationDetails();
    ...
    private void addOuterThickBorders(Sheet sheet, MutableInt rowId) {
        CellRangeAddress region = CellRangeAddress.valueOf("B2:K" + rowId.intValue());
