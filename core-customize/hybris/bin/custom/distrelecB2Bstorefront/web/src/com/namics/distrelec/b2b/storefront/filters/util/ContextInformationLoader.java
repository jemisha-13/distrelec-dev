 ...
 *  
 */
package com.namics.distrelec.b2b.storefront.filters.util;
...
public class ContextInformationLoader
{
	...
	protected void loadFakeLanguage(final LanguageModel languageModel)
	{
		if (languageModel != null)
		{
			...
			LanguageModel usedLanguage = commonI18NService.getAllLanguages().stream()
					.filter(language -> language.getIsocode().equals(languageModel.getIsocode()+"_"+currentCountryIsocode))
					.findFirst().orElse(languageModel);
			String language = usedLanguage.getIsocode();
			getI18NService().setCurrentLocale(LocaleUtils.toLocale(language));
		}
	}
