...
@Component
public class DistQualityAndLegalPdfExportHelper {
    ...
    public void getPDFStreamForQualityAndLegalReport(List<ProductData> products, OutputStream output) throws IOException, JRException {
        List<ProductData> b2bProducts = new ArrayList<>();
        products.forEach(productData -> b2bProducts.add(productData));
    ...
    private void addCustomerParams(final Map<String, Object> parameters) {
        CustomerData b2bCustomer = b2bCustomerFacade.getCurrentCustomer();
        parameters.put("customerName", b2bCustomer.getFirstName() + " " + b2bCustomer.getLastName());
