...
public class ProductCarouselComponentControllerTest
{
	...
	public void testFillModel()
	{
		...
		given(
				productFacade.getProductForOptions(Mockito.same(productModelProducts),
						(List<ProductOption>) argThat(new OptionsMatcher()))).willReturn(productDataProducts);
		given(
				productFacade.getProductForOptions(Mockito.same(productModelCategories),
						(List<ProductOption>) argThat(new OptionsMatcher()))).willReturn(productDataCategories);
		...
		verify(productFacade).getProductForCodeAndOptions(Mockito.eq(productModelProducts.getCode()),
				Mockito.anyList());
		verify(productFacade).getProductForCodeAndOptions(Mockito.eq(productModelCategories.getCode()),
				Mockito.anyList());
	...
	private static class OptionsMatcher implements ArgumentMatcher
	{
		@Override
