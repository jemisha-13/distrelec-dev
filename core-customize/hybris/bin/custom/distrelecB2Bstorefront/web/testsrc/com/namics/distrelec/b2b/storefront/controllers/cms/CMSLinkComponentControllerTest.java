...
public class CMSLinkComponentControllerTest {
    ...
    private static final String URL = "localizedUrl";
    ...
    public void setUp() {
        ...
        given(request.getSession())
                .willReturn(session);
        given(session.getServletContext())
                .willReturn(servletContext);
        given(servletContext.getAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE))
                .willReturn(webApplicationContext);
        ...
        ReflectionTestUtils.setField(cmsLinkComponentController, "contentPageUrlConverter", contentPageUrlConverter);
    }

    @Test
    public void testRenderComponentWithComponentUrl() throws Exception {
        given(cmsLinkComponentModel.getLocalizedUrl()).willReturn(TEST_COMPONENT_URL);
    ...
    public void testRenderComponentWithContentPage() throws Exception {
        ...
        given(contentPageUrlConverter.convert(contentPageModel)).willReturn(contentPageData);
        given(contentPageData.getUrl()).willReturn(TEST_CONTENT_PAGE_LABEL);
        given(cmsLinkComponentModel.getItemtype()).willReturn(TEST_TYPE_CODE);
    ...
    public void testRenderComponentUid() throws Exception {
        ...
        given(cmsComponentService.getAbstractCMSComponent(TEST_COMPONENT_UID)).willReturn(cmsLinkComponentModel);
        given(cmsLinkComponentModel.getProduct()).willReturn(productModel);
        given(productUrlConverter.convert(productModel)).willReturn(productData);
        given(productData.getUrl()).willReturn(TEST_PRODUCT_URL);
        given(cmsLinkComponentModel.getItemtype()).willReturn(TEST_TYPE_CODE);
