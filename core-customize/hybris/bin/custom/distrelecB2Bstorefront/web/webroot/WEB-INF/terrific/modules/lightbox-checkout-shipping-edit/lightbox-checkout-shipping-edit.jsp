...
<spring:theme code="checkout.address.buttonEdit" text="Add shipping address" var="sTitle" />

<div class="modal-backdrop hidden">&nbsp;</div>

<div class="modal hidden fade" tabindex="-1" role="dialog">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<c:if test="${isEShopGroup eq false}">
				<c:choose>
					<c:when test="${customerType eq 'B2B' || customerType eq 'b2b' || customerType eq 'B2B_KEY_ACCOUNT'}">
						<c:url value="/checkout/detail/createUpdate/b2b/shipping" var="changeAddressUrl" />

						<form:form class="col-12 box-address__form" method="post" modelAttribute="B2BShippingAddressForm_${address.id}" action="${changeAddressUrl}">
							...
							<input type="hidden" id="formId" name="formId" value="${addressIndex}" />
							<div class="box-address__form__field">
								<formUtil:formInputBox idKey="companyName" path="companyName" placeHolderKey="checkoutregister.companyName" maxLength="35" value="${address.companyName}" />
					...
					<c:otherwise>
						<c:url value="/checkout/detail/createUpdate/b2c/shipping" var="changeAddressUrl" />
                        <c:if test="${not empty b2CShippingForm}">
                            <form:form class="col-12 box-address__form" method="post" modelAttribute="b2CShippingForm_${address.id}" action="${changeAddressUrl}">
                                <input type="hidden" id="addressId" name="addressId" value="${address.id}" />
                                <input type="hidden" id="shippingAddress" name="shippingAddress" value="true" />
								<input type="hidden" id="formId" name="formId" value="${addressIndex}" />
                                <div class="box-address__form__select">
