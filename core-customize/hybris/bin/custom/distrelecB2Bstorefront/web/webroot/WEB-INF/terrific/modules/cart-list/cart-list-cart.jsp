...
<div class="skin-cart-list-cart__header">
	<div class="cell cell-availability">
		<p>
			<spring:message code="cart.list.product" />
		</p>
	</div>
	...
	<c:if test="${showMyPrice and showListPrice}">
		<div class="cell cell-price two-prices">
			<div class="price-row">
...
<div class="skin-cart-list-cart__content cart-list loading">
	...
	<c:forEach var="orderEntry" items="${cartData.entries}" varStatus="status">
		...
		<mod:cart-list-item template="quote-head" tag="div" orderEntry="${orderEntry}" htmlClasses="quote-head skin-cart-list-item-cart quote-${orderEntry.quotationId}" isBOM="${isBOM}" />
		<c:set var="currentQuote" value="${orderEntry.quotationId}" />
		</c:if>
		<mod:cart-list-item tag="div" orderEntry="${orderEntry}" showMyPrice="${showMyPrice}" showListPrice="${showListPrice}" template="cart" skin="cart"
							htmlClasses="${BOMClass}${quoteClass}${dummyClass} quote-item-checkout-row-${isQuoteItem} js-cart-list-item-${orderEntry.product.codeErpRelevant}"
