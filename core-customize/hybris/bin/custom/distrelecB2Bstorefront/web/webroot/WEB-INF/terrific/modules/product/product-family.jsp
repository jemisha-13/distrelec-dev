...
<div class="product-family">
    <div class="product-family__aside">
        ...
        <a class="product-family__aside--brand" data-aainteraction="family menu click" data-link-text="Scroll To Product List">
            <c:if test="${not empty searchPageData}">
                <li class="scroll-to focus_on_list" data-scroll="listing"><spring:message code="product.family.product.list" /></li>
            </c:if>
            <spring:message code="product.family.product.list" />
        </a>
    </div>
            ...
            <c:if test="${not empty productFamily.familyManufacturerImage}">
                <c:forEach items="${productFamily.familyManufacturerImage}" var="item">
                    ...
                    <c:if test = "${fn:endsWith(imageName, '_bl')}">
                        <div class="img-wrap">
                            <a rel="noopener noreferrer" href="${manufacturerUrl}" target="_self">
        ...
        <div class="jump-to">
            ...
            <ul>
                <c:if test="${not empty productFamily.familyBullets}">
                    <li data-aainteraction="family menu click" data-link-text="Features" class="scroll-to" data-scroll="feature"><spring:message code="product.family.features" /></li>
                </c:if>
                <c:if test="${not empty productFamily.familyApplications}">
                    <li data-aainteraction="family menu click" data-link-text="Applications" class="scroll-to" data-scroll="applications"><spring:message code="product.family.applications" /></li>
                </c:if>
                <c:if test="${not empty productFamily.familyMedia}">
                    <li data-aainteraction="family menu click" data-link-text="Additional information" class="scroll-to" data-scroll="schematics"><spring:message code="product.family.schematics" /></li>
                </c:if>
                <c:if test="${not empty searchPageData}">
                    <li data-aainteraction="family menu click" data-link-text="Product listing" class="scroll-to" data-scroll="listing"><spring:message code="product.family.product.list" /></li>
                </c:if>
            </ul>
        </div>
            ...
            <div class="product-family__main--features" id="feature">
                ...
                <c:if test="${not empty productFamily.familyDatasheet}">
                    <c:forEach items="${productFamily.familyDatasheet}" var="datasheet">
                        <a href="${datasheet.value.url}" data-aainteraction="file download" data-file-name="${datasheet.value.name}.pdf" target="_blank" title="${datasheet.value.name}"><spring:message code="product.family.view.datasheet" /></a>
                    </c:forEach>
                </c:if>
            </div>
        </c:if>
