...
<json:object>
	...
	<json:array name="products" items="${products}" var="product">
		<json:object>
			...
			<json:property name="pimAlternateCategory" value="${product.alternateCategory}" />
			...
			<json:object name="price">
				...
				<json:property name="formattedValue">
					<c:forEach items="${product.volumePricesMap}" var="volumePriceEntry" end="0">
						<c:if test="${not empty volumePriceEntry.value.custom}">
							<c:set var="hasCustomPrices" value="${true}" />
							<c:set var="yourPriceDiffers" value="${volumePriceEntry.value.list.value gt volumePriceEntry.value.custom.value}" />
						</c:if>
						<c:choose>
							<c:when test="${yourPriceDiffers or hasCustomPrices}">
								<format:price format="price" priceData="${volumePriceEntry.value.custom}" />
							</c:when>
							<c:otherwise>
