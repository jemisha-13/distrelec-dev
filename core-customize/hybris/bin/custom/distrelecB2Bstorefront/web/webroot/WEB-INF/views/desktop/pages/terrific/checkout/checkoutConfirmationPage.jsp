...
<spring:theme code="checkout.orderComplete.continueLink" text="Continue shopping" var="sContinueShopping" />
		...
		<div class="container">
			<div class="row">
				...
				<c:choose>
					<c:when test="${isB2E}">
						<div class="col-12">
							<mod:checkout-success orderCode="${order.code}" userId="${user.email}" />
							<mod:checkout-consent />
						</div>
						<div class="col">
							<a class="return-btn" href="${homeUrl}">${sContinueShopping}</a>
						</div>
					</c:when>
					<c:otherwise>
						<div class="col-12 ${user.customerType ne 'GUEST' ? 'col-md-9' : ''}">
							<mod:checkout-success orderCode="${orderCode}" userId="${user.email}" />
							<mod:checkout-consent />
						</div>
						<c:if test="${user.customerType ne 'GUEST'}">
