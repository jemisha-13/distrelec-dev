	...
	<div class="imagery-holder ${imageFull}">
		...
		<c:forEach items="${product.productImages}" var="productImage" varStatus="status" begin="0" end="0">
			...
			<div data-slide-id="zoom-item" class="zoom-gallery-slide active">
				<%@ include file="/WEB-INF/terrific/modules/OCI/product-image-gallery/gallery-item.jsp" %>
			</div>
		</c:forEach>
		...
		<c:forEach items="${product.images360}" var="productImage360" varStatus="status">
			<c:set var="imageIndex" value="${status.index}" />
			<%@ include file="/WEB-INF/terrific/modules/OCI/product-image-gallery/gallery-item-360.jsp" %>
		</c:forEach>
	...
	<c:if test="${imageLength > 1}">
		<div class="gallery-side MagicScroll" id="gal1" data-options="orientation: vertical">
			...
			<c:forEach items="${product.productImages}" var="productImage" varStatus="status" begin="0" end="0">
				...
				<c:set var="landscapeLargeWebp" value="${not empty productImage.landscape_large_webp.url ? productImage.landscape_large_webp.url : not empty productImage.landscape_large.url ? productImage.landscape_large.url : '/_ui/all/media/img/missing_landscape_medium.png'}" />
				...
				<a data-slide-id="zoom-item" data-zoom-id="zoom-item-v" href="${landscapeLargeWebp}" class="item active current${status.index eq 0 and product.illustrativeImage ? ' illustrative-image' : ''}" data-image="${landscapeLargeWebp}" data-index="${imageIndex}">
					<img width="51" height="68" alt="${portraitSmallAltText}" src="${landscapeLargeJpg}">
				</a>
			</c:forEach>
			...
			<c:forEach items="${product.productImages}" var="productImage" varStatus="status" begin="1" end="${fn:length(product.productImages)}">
				...
				<c:set var="portraitSmallAltText" value="${not empty productImage.portrait_small_webp.altText ? productImage.portrait_small_webp.altText : not empty productImage.portrait_small.altText ?  productImage.portrait_small.altText : sImageMissing}" />
				<a data-slide-id="zoom-item" data-zoom-id="zoom-item-v" href="${landscapeLargeJpg}" class="item" data-index="${imageIndex}" data-image="${landscapeLarge}">
