...
<ul class="cart-list loading">
	<c:forEach var="orderEntry" items="${orderData.entries}" >
		...
		<mod:cart-list-item template="approval-request" skin="order-detail skin-order-detail-approval-request" tag="li" htmlClasses="row${isDummyItem ? ' dummy-item' : '' }" orderEntry="${orderEntry}" isDummyItem="${isDummyItem}" />
	</c:forEach>
</ul>
...
<script id="tmpl-stock_level" type="text/x-dot-template">
	...
	<div class="row">
		<div class="left">> 10d{{= it.backorderQuantityFormated}}</div>
