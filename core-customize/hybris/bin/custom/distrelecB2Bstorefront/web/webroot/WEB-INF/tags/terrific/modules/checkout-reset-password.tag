...
<terrific:mod name="checkout-reset-password" tag="${tag}" htmlClasses="${htmlClasses}" skin="${skin}" dataConnectors="${dataConnectors}" attributes="${attributes}">
	...
	<c:choose>
		<c:when test="${isOCI eq false}">
			<%@ include file="/WEB-INF/terrific/modules/checkout-reset-password/checkout-reset-password.jsp" %>
		</c:when>
		<c:otherwise>
			<%@ include file="/WEB-INF/terrific/modules/OCI/checkout-reset-password/checkout-reset-password.jsp" %>
		</c:otherwise>
	</c:choose>
</terrific:mod>
