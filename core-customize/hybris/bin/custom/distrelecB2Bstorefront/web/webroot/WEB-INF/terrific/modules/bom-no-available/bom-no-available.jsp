...
<spring:message code='bom.notstocked.message.info' arguments="" var="notstockedMessageInfo"/>
	...
	<c:forEach items="${unavailableProducts}" var="unavailableProduct">
		...
		<c:choose>
			<c:when test="${fn:length(productReferences) gt 0}">
				...
				<div class="bom-unavailable-product" data-product-code="${unavailableProduct.productCode}">
					<mod:global-messages template="component" skin="component info bom-product"  headline='' body='${notstockedMessageInfo}' type="info"/>

					<div class="bom-unavailable-product__info">

						<div>
							<div class="bom-unavailable-product__info-box bom-product">
								<span class="bom-unavailable-product__original-request"><strong><spring:message code="text.original.request" />:</strong></span>
								<span class="bom-unavailable-product__title">${unavailableProduct.quantity}, ${unavailableProduct.searchTerm} - ${unavailableProduct.product.name}, ${unavailableProduct.reference}</span>
							</div>

							<button class="mat-button mat-button--action-blue bom-unavailable-product__show-alternative"
									...
									data-product-code="${unavailableProduct.productCode}" data-show-text="<spring:message code="text.showalternativeitems" />"
									data-hide-text="<spring:message code="text.hidealternativeitems" />" data-change-text="<spring:message code="text.changealternativeitems" />">
								<spring:message code="text.showalternativeitems" />
							</button>

						</div>
					</div>

					<c:forEach items="${productReferences}" var="alternativeproduct" varStatus="lStatus">
						<div class="bom-unavailable-product__alternatives-selection bom-unavailable-product__item bom-unavailable-product__item--${alternativeproduct.target.code}">
							<mod:product template="bom" skin="bom" tag="div" product="${alternativeproduct.target}" producttype="alternative" carriedReference="${unavailableProduct.reference}" carriedQuantity="${unavailableProduct.quantity}"/>
						</div>
					</c:forEach>

					<div class="row bom-unavailable-product__alternatives hidden">

						<c:forEach items="${productReferences}" var="product" varStatus="lStatus">

							<div class="col-12 col-sm-4 col-lg-3 card-item bom-unavailable-product__item bom-unavailable-product__item--${product.target.code}">

								<div class="card-item__items">
									<div class="card-item__items__image">
										<c:set var="productImage" value="${product.target.productImages[0]}"/>
									...
									<div class="card-item__items__modal">
										...
										<button type="button" id="viewdetails-${product.target.code}" data-toggle="modal" data-product-code="${product.target.code}"
												data-target="#modal-${product.target.code}"
												class="bom-unavailable-product__view-detail mat-button mat-button__normal--action-blue" >
												${sViewDetails}</button>

										<!-- Modal -->
										<div id="modal-${product.target.code}" class="modal fade modal-${product.target.code}" role="dialog">
															...
															<div class="modal-content card-item__items__modal__content">
																...
																<div class="modal-content modal__body">
																	...
																	<div class="modal-content modal__product-information modalInformation">
																		<p class="modalArticle">${sArticleNumberText}: <strong class="ArticleResponse">${product.target.code}</strong></p>
																		<p class="modalMPN">${sTypeNumberText}: <strong class="typeNameResponse">${product.target.typeName}</strong></p>
									...
									<div class="card-item__items__content">
										<a class="card-item-anchor" href="${product.target.url}" data-aainteraction="bom alternative click" data-product-id="${product.target.code}" data-position="${lStatus.count}" data-pagination="Grid" data-component="${componentId}" data-scenarioID="${recoType}" data-stock="${stockLevels[product.target.code]}">
										...
										<div class="card-item__items__content__product-info">
											<p class="card-item__items__content__product-info__item"><strong>${sManufacturer}:</strong> ${product.target.distManufacturer.name}</p>
											<p class="card-item__items__content__product-info__item"><strong>${sArticle}:</strong> ${product.target.codeErpRelevant} </p>
										</div>

										<div  class="stock-${product.target.code} card-item__items__content__stock">
											<mod:erp-sales-status productArtNo="${product.target.code}" productStatusCode="${product.target.salesStatus}"/>
										</div>
										<p class="card-item__items__content__price">
											<span class="card-item__items__content__price__item"><format:fromPrice priceData="${product.target.price}"/></span>
											<span class="card-item__items__content__price__item--small">(${sExclVat})</span>
										</p>

										<button class="bom-unavailable-product__alternatives-select mat-button mat-button__solid--action-green ellipsis"
												data-aainteraction="select alternative" data-product-code="${product.target.code}" title='${sSelect}' text="Select">
