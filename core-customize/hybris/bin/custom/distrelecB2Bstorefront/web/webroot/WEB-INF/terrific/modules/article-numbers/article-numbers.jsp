	...
	<c:otherwise>
		<div class="elem elem--first js-product-code" data-product-code="<format:articleNumber articleNumber="${product.codeErpRelevant}" />">
			...
			<sec:authorize access="hasRole('ROLE_CUSTOMERGROUP')">
				<c:if test="${product.availableInSnapEda}">
					<script src="https://www.snapeda.com/distributor/files/plugins/snapeda/snapeda.min.js?token=21e330fad7e23375a2e0e3fb3f72f6" defer autoload part="${product.typeName}" manufacturer="${distManufacturer.name}" container=".message-container-gallery" token="21e330fad7e23375a2e0e3fb3f72f6" host="https://www.snapeda.com" lang="${currentLanguage.getIsocode()}"></script>
				</c:if>
			</sec:authorize>
