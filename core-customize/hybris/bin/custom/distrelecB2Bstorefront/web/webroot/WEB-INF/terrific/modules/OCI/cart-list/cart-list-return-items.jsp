...
<div id="error-field" class="error-message hidden">
...
<form:form modelAttribute="createRMARequestForm"  method="post" action="${returnOnlineAction}" cssClass="form-rma">
	...
	<ul class="cart-list loading border-bottom-grey">
		<c:forEach var="orderEntry" items="${orderData.entries}" varStatus="status">
			...
			<mod:cart-list-item orderData="${orderData}" template="return-items" skin="return-items" tag="li" htmlClasses="row cart-list-item${isDummyItem ? ' dummy-item' : '' }" orderEntry="${orderEntry}" loopNumber="${status.index}" isDummyItem="${isDummyItem}" />
		</c:forEach>
	</ul>
	...
	<div class="row return-items-buttons">
		...
		<c:if test="${canSubmitRmaRequest}">
            <div class="gu-9">
                <button class="btn btn-primary btn-return-items"
