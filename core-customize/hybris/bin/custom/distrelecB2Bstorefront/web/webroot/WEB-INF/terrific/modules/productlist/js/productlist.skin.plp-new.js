(function($) {
     ...
     *
     */
    Tc.Module.Productlist.PlpNew = function(parent) {
        this.on = function(callback) {
            ...
            //toggle product
            this.$toggleProduct = this.$ctx.find(".plp-filter-products__product__right");
            self.$toggleProduct.off("click").on("click", function(ev) {
                ev.preventDefault();
                ev.stopPropagation();

                $(this).parents('.plp-filter-products__product').find('.plp-filter-products__product__info__detailed').toggleClass('showdetailview');
                $(this).toggleClass('showdetailview');

                var $detailsText = $(this).parents('.plp-filter-products__product').find('.plp-filter-products__product__right.sidebar-plp');
                $detailsText.children('.plp-filter-products__product__right__show-details').toggleClass('hide');
                $detailsText.children('.plp-filter-products__product__right__hide-details').toggleClass('hide');
            });

            this.$toggleProductLeft = this.$ctx.find(".plp-filter-products__product__right.sidebar-plp");
            self.$toggleProductLeft.off("click").on("click", function(ev) {

                $(this).parents('.plp-filter-products__product').find('.plp-filter-products__product__info__detailed').toggleClass('showdetailview');
                $(this).children('.plp-filter-products__product__right__toggle').toggleClass('showdetailview');
                $(this).children('.plp-filter-products__product__right__show-details').toggleClass('hide');
                $(this).children('.plp-filter-products__product__right__hide-details').toggleClass('hide');
        ...
        this.triggerCart = function($product, queryParam) {
            ...
            if (document.URL.indexOf('?') !== -1) {
                ...
                filterApplied = filterApplied.replace(' ', '+');
            }
        ...
        this.getPopover = function(item, $listItem, productCode) {
            ...
            $.each(item.stockLevels, function(index, stockLevel) {
                // Waldom check
                if ((stockLevel.isWaldom && stockLevel.mview === "BTR") || stockLevel.mview === "DIR") {

                    $plpProductItem.find('.mod-numeric-stepper .ipt').data('max-waldom-stock', stockLevel.available);

                    if(stockLevel.available === 0 && stockLevel.mview === "DIR"){

                        if (statusCode >= 40) {
                            $('.button-holder--' + productCode + ' button').addClass('disabled');
                            $infoStock.find(".availableToOrder").addClass("hidden");
    
                            $(".info-stock-" + productCode + " .sales-status--status-text-40-45").removeClass("hidden");
                            $(".info-stock-" + productCode + " .sales-status--status-text-40-45 .outofstock-41-45").removeClass("hidden");
    
                         }

                    }else if(stockLevel.available > 0 ) {
                        var $inStockWaldom = $infoStock.find(".inStockText");

                        if ($inStockWaldom.data("instock-text") !== undefined) {
                            var inStockTextWaldom = $inStockWaldom
                                .data("instock-text")
                    ...
                    } else if( item.stockLevelTotal === 0 && stockLevel.mview !== "DIR"){
                        var $outofStock = $infoStock.find('.left.out-of-stock.availableToOrder');

                        $plpProductItem
                            .find(".plp-filter-products__cart-cta .btn-cart")
                            .addClass("disabled")
                            .attr("disabled", "disabled");
                        $plpProductItem.find(".mod-numeric-stepper").addClass("disabled");
                        $plpProductItem.find(".numeric-btn").addClass("disabled");

                        $.each($outofStock , function(index, outofStockWaldom){
                            outofStockWaldom.innerHTML = '<i class="fas fa-times-circle"></i><span class="left">' + $outofStock.data('waldom-unavailable') + '</span>';
                        });
                    }

                } else if ((stockLevel.mview === "BTO" || stockLevel.mview === "DIR") && item.stockLevelTotal === 0){
                    ...
                    if (statusCode >= 40) {
                        ...
                        $infoStock.find(".currentlyNotAvailable").addClass("hidden");
                    }
                ...
                // In Stock
                        ...
                        if (stockLevel.available > 0) {
                            ...
                            $infoStock
                                .find(".sales-status--status-text-40-45")
                                .addClass("hidden");
                        } else if(statusCode === 53 || statusCode === 52 || statusCode === 50) {
                            $infoStock.find('.sales-status--block').removeClass('hidden');
                            $infoStock.find(".availableToOrder").addClass("hidden");
                        } else {
                            $infoStock.find(".availableToOrder").removeClass("hidden");
                        }
                        ...
                        var disabledStatuses = [50,51,52,53];
                        var $addToCartBtn = $infoStock.closest('.js-plp-product-list-item').find('.js-btn-cart');

                        if(disabledStatuses.indexOf(statusCode) !== -1 && item.stockLevelTotal === 0) {
                            $addToCartBtn.attr('disabled', true);
                        }

                        if(item.stockLevelTotal === 0 && stockLevel.mview === "SC" && (statusCode === 30 || statusCode === 31)) {
                            $infoStock.find(".availableToOrder").addClass("hidden");
                            $infoStock.find(".currentlyNotAvailable").removeClass("hidden");
                        }

                        if(item.stockLevelTotal > 0 && stockLevel.mview === "SC" && (statusCode === 60 || statusCode === 61)) {
                            $(".info-stock-" + productCode + " .sales-status--status-text-40-45").removeClass("hidden");
                            $(".info-stock-" + productCode + " .sales-status--status-text-40-45 .outofstock-41-45").removeClass("hidden");
                        }
                        ...
                        if(inStock === true) {
                            ...
                            switch(statusCode) {
                                ...
                                case 62:
                                    $infoStock.find('.sales-status--block').removeClass('hidden');
                                    $infoStock.find('.instock').addClass('hidden');
                                    $infoStock.find('.availableToOrder').addClass('hidden');
                                    break;
                                case 30:
                                case 31:
                                ...
                                case 41:
                                    $infoStock.find('.instock').removeClass('hidden');
                                    $infoStock.find('.instock-41-45').addClass('hidden');
                                    $infoStock.find('.availableToOrder').addClass('hidden');
                                    break;
                                case 50:
                                case 51:
                                case 52:
                                case 53:
                                    $infoStock.find('.sales-status--block').addClass('hidden');
                                    $infoStock.find('.availableToOrder').addClass('hidden');
                                    $infoStock.find('.sales-status--status-text-40-45').removeClass('hidden');
                                    $infoStock.find('.outofstock-41-45').addClass('hidden');
                                    break;
                                case 90:
                                case 91:
                                    $infoStock.find('.sales-status--block').addClass('hidden');
                                    $infoStock.find('.availableToOrder').addClass('hidden');
                                    $infoStock.find('.sales-status--status-text-40-45').removeClass('hidden');
                                    $infoStock.find('.outofstock-41-45').removeClass('hidden');
                                    $infoStock.find('.instock').addClass('hidden');
                                    break;
                            }
                        ...
                        var itemCategoryGrps = ['BANS','BANC'],
                            itemCategoryGrp = $('.product-code-'+productCode).data('item-catgroup'),
                            allPlpProducts = $('.hidden_product_code_compare');

                        if(itemCategoryGrps.indexOf(itemCategoryGrp) !== -1 && (statusCode === 30 || statusCode === 31) ) {
                        ...
                        allPlpProducts.each(function( index, product ) {
                            var  $product = $(product)
                                ,$productCategoryGrp = $product.data('item-catgroup')
                            ;

                            digitalData.product[index].productInfo.shipType = $productCategoryGrp;
                        });
                ...
                if (item.stockLevelTotal > 0) {
                    ...
                    var $further = $infoStock.find(".inStockText");

                    if ($further.data("instock-text") !== undefined) {
                        var $furtherText = $further
                            .data("instock-text")
                            .replace("{0}", item.stockLevelTotal);
