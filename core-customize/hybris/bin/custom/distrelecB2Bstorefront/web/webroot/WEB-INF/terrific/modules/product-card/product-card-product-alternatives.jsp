...
<div id="alternativesVm" class="alternatives-holder product-alternatives-vm product-alt" data-product-code="${product.codeErpRelevant}" v-cloak
...
>
    ...
    <div class="product-alternatives-vm__list">
        ...
        <product-alternatives v-if="index < itemsToShow" v-for="(item, index) in alternativesData"
                              v-bind:index="index"
    ...
    <button class="mat-button mat-button__normal--action-blue alternatives-holder__all-alternatives hidden">
        <span class="">${viewMorealternativeText} <i class="fas fa-angle-right"></i></span>
    </button>

    <button v-if="alternativesData.length > itemsToShow" class="mat-button mat-button--action-green" @click="itemsToShow += 8">${viewMoreProductsText}</button>
