	...
	<c:if test="${not empty product.rohs || not empty product.svhc || not empty product.svhcReviewDate || isDangerousGoods eq true}">
		<li class="list-holder__item list-holder__item--environment-list">
				...
				<c:when test="${isRNDProduct}">
					<c:if test="${isROHSConform}">
						<div class="holder-content flex-holder">
							...
							<a class="link" title="<spring:message code="detailtabsproductinformation.rohs.linkname" text="Order Note" />" href="/compliance-document/pdf/ROHS_${product.code}" data-aainteraction="file download" data-file-type="pdf" data-file-name="ROHS_${product.code}.pdf" id="ROHS-pdf-link" target="_blank"><spring:message code="detailtabsproductinformation.rohs.linkname" /></a>
						</div>
					</c:if>
				</c:when>
				<c:otherwise>
			...
			<c:if test="${not empty product.hasSvhc}">
				...
				<div class="holder-content flex-holder">
					<span id="REACH-id" class="title"><spring:message code="reach.regulation.title" /></span>
					<span id="REACH-title-id">${reachRegulationName}</span>
				</div>
				<div class="holder-content flex-holder">
					<span id="REACH-id-empty" class="title title__empty">&nbsp;</span>
					<a class="link" data-aainteraction="file download" data-file-type="pdf" data-file-name="SVHC_${product.code}.pdf" target="_blank" title="${svhcLinkText}" href="/compliance-document/pdf/SVHC_${product.code}">${svhcLinkText}</a>
				</div>
			</c:if>
