...
<template:errorSpanField path="${path}" errorPath="${errorPath}">
	...
	<%-- autocomplete is off because if page reloads, they should be empty and pw indicator is set to 0 --%>
	<form:password autocomplete="off" cssClass="field ${inputCSS} ${cssErrorClass}" placeholder="${placeHolderText}" id="${idKey}" path="${path}" tabindex="${tabindex}" />
