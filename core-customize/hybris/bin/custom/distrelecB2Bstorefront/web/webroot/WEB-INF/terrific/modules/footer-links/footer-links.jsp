...
<spring:theme code="footer-links.title" text="Direct Access"
	var="sTitle" />
...
<c:choose>
	...
	<c:otherwise>
		<c:forEach items="${navigationNodes}" var="level1node" varStatus="status">
			...
			<section class="nav-footer-section col-md-4 col-lg  ${footerTitle} ">
				...
				<ul class="nav-footer-list">
					<c:if test="${level1node.name =='Contact'}">
						<c:forEach items="${level1node.entries}" var="entries" begin="0" end="1">
							<li><span>${entries.htmlContent.content}</span></li>
						</c:forEach>
					</c:if>
