...
public class MultipleCatalogsSyncJobPerformableTest {
    ...
    public void prepare() {
        ...
        performable = new MultipleCatalogsSyncJobPerformable() {
            @Override
            protected de.hybris.platform.commerceservices.setup.SetupSyncJobService getSetupSyncJobService() {
                return setup;
            }
        };
    ...
    public void testProductAndContentCatalogAllReturningSuccess() {
        ...
        BDDMockito.given(setup.executeCatalogSyncJob(Mockito.anyString())).willReturn(new PerformResult(CronJobResult.SUCCESS, CronJobStatus.FINISHED));
    ...
    public void testProductAndContentCatalogAssignFails() {
        ...
        Mockito.doThrow(new IllegalStateException("expected")).when(setup).assignDependentSyncJobs(PRODUCT_CATALOG, Collections.singleton(CONTENT_CATALOG));
    ...
    public void testProductAndContentCatalogFirstExecutionFails() {
        ...
        BDDMockito.given(setup.executeCatalogSyncJob(CONTENT_CATALOG)).willReturn(new PerformResult(CronJobResult.FAILURE, CronJobStatus.ABORTED));
        BDDMockito.given(setup.executeCatalogSyncJob(PRODUCT_CATALOG)).willReturn(new PerformResult(CronJobResult.SUCCESS, CronJobStatus.FINISHED));
    ...
    public void testProductAndContentCatalogLastExecutionFails() {
        ...
        BDDMockito.given(setup.executeCatalogSyncJob(CONTENT_CATALOG)).willReturn(new PerformResult(CronJobResult.SUCCESS, CronJobStatus.FINISHED));

        BDDMockito.given(setup.executeCatalogSyncJob(PRODUCT_CATALOG)).willReturn(new PerformResult(CronJobResult.FAILURE, CronJobStatus.ABORTED));
