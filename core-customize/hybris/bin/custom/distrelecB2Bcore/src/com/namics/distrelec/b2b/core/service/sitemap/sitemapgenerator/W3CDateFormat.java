...
/**
 * <p>
 * Formats and parses dates in the six defined W3C date time formats. These formats are described in "Date and Time Formats", <a
 ...
 * </ol>
 * 
 * Note that W3C timezone designators (TZD) are either the letter "Z" (for GMT) or a pattern like "+00:30" or "-08:00". This is unlike RFC
 * 822 timezones generated by SimpleDateFormat, which omit the ":" like this: "+0030" or "-0800".
 * </p>
 * 
 * <p>
 * This class allows you to either specify which format pattern to use, or (by default) to automatically guess which pattern to use (AUTO
 * mode). When parsing in AUTO mode, we'll try parsing using each pattern until we find one that works. When formatting in AUTO mode, we'll
 * use this algorithm:
 * 
 * <ol>
 ...
 * </ol>
 * 
 * Finally note that, like all classes that inherit from DateFormat, <b>this class is not thread-safe</b>. Also note that you can explicitly
 * specify the timezone to use for formatting using the {@link #setTimeZone(TimeZone)} method.
 * 
 * @author Dan Fabulich
