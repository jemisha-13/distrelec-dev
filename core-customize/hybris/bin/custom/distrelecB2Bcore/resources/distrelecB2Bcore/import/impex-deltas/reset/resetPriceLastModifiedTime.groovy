...
@Field Logger LOG = LoggerFactory.getLogger("PRICE_ERP_TIME_RESET_SCRIPT")

@Field int batchSize = 100000

def getSelectQuery() {
    if (Config.isSQLServerUsed()) {
        return "SELECT TOP $batchSize pk FROM pricerows WHERE p_lastmodifiederp IS NOT NULL"
    } else {
        return "SELECT pk FROM pricerows WHERE p_lastmodifiederp IS NOT NULL ${getLimitQuery()}"
    }
}
...
def getTotalCount() {
    Statement statement = connection.prepareStatement("SELECT COUNT(*) FROM pricerows WHERE p_lastmodifiederp IS NOT NULL")
    try {
...
def updateNextBatch() {
    String query = """UPDATE pricerows SET p_lastmodifiederp = NULL WHERE pk IN (${getSelectQuery()})"""
