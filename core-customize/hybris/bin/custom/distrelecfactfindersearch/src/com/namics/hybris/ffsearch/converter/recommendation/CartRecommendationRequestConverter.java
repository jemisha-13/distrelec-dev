...
import de.factfinder.webservice.ws71.FFrecommender.ArrayOfString;
 ...
 */
public class CartRecommendationRequestConverter extends AbstractPopulatingConverter<CartRecommendationRequest, RecommendationResponse> {

    private static final Logger LOG = LoggerFactory.getLogger(CartRecommendationRequestConverter.class);
    ...
    @Override
    public void populate(final CartRecommendationRequest source, final RecommendationResponse target) {
        if (source.getProductCodes() == null || source.getProductCodes().isEmpty()) {
            target.setRecommendations(new RecommenderResult());
        } else {
            try {
                // current integration with user info = null
                // UserInformation userInfo = new UserInformation();

                final ArrayOfString productCodes = new ArrayOfString();
                productCodes.getString().addAll(source.getProductCodes());
                RecommenderPortType port =getRecommendationWebserviceClient();
                final RecommenderResult recommendationResult = port.getRecommendation1(10, productCodes, source.getChannel(),
                        false, getAuthentication().getRecommendationAuth());
                readAndStoreCookie(((BindingProvider) port));
                target.setRecommendations(recommendationResult);
            } catch (final Exception e) {
                LOG.error("Unable to fetch campaign for products [{}] in FactFinder.", source.getProductCodes(), e);
                target.setRecommendations(new RecommenderResult());
            }
        }
