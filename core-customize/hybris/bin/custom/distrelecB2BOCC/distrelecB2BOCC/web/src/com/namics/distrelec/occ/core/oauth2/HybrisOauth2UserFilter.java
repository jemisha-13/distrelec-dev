 ...
 */
package com.namics.distrelec.occ.core.oauth2;
 ...
 * This should happen only when there is a customer context. Anonymous credentials are also applicable, because special 'anonymous' user is
 * available for that purpose. Customer context is not available during client credential flow.
 */
public class HybrisOauth2UserFilter implements Filter {
    ...
    public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain chain) throws IOException, ServletException {
        ...
        if (containsRole(auth, ROLE_ANONYMOUS) || containsRole(auth, ROLE_CUSTOMERGROUP) || containsRole(auth, ROLE_CUSTOMERMANAGERGROUP)) {
