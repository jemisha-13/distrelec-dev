...
public class CatalogsController extends BaseController {
    ...
    public CatalogListWsDTO getCatalogs(@ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields) {
        ...
        return getDataMapper().map(catalogsData, CatalogListWsDTO.class);
    }
    ...
    @ApiBaseSiteIdParam
    public CatalogVersionWsDTO getCatalogVersion(@Parameter(description = "Catalog identifier", required = true) @PathVariable final String catalogId,
                                                 @Parameter(description = "Catalog version identifier", required = true) @PathVariable final String catalogVersionId,
                                                 @ApiFieldsParam @RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields) {
        final CatalogVersionData catalogVersionData = catalogFacade.getProductCatalogVersionForTheCurrentSite(catalogId, catalogVersionId, OPTIONS);

        final FieldSetBuilderContext context = new FieldSetBuilderContext();
        context.setRecurrencyLevel(countRecurrencyForCatalogVersionData(catalogVersionData));
        final Set<String> fieldSet = fieldSetBuilder.createFieldSet(CatalogVersionWsDTO.class, DataMapper.FIELD_PREFIX, fields, context);
    ...
    @ApiBaseSiteIdParam
    public CategoryHierarchyWsDTO getCategories(@Parameter(description = "Catalog identifier", required = true) @PathVariable final String catalogId,
                                                @Parameter(description = "Catalog version identifier", required = true) @PathVariable final String catalogVersionId,
        ...
        final CategoryHierarchyData categoryHierarchyData = catalogFacade.getCategoryById(catalogId, catalogVersionId, categoryId, page, OPTIONS);

        final FieldSetBuilderContext context = new FieldSetBuilderContext();
        context.setRecurrencyLevel(countRecurrencyForCategoryHierarchyData(1, categoryHierarchyData));
        final Set<String> fieldSet = fieldSetBuilder.createFieldSet(CategoryHierarchyWsDTO.class, DataMapper.FIELD_PREFIX, fields, context);
