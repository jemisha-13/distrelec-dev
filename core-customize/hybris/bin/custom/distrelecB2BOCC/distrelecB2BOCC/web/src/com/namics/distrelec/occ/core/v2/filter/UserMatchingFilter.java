...
public class UserMatchingFilter extends AbstractUrlMatchingFilter {
    ...
    protected void doFilterInternal(final HttpServletRequest request, final HttpServletResponse response, final FilterChain filterChain)
        ...
        if (hasRole(SecuredAccessConstants.ROLE_CUSTOMERGROUP, auth) || hasRole(SecuredAccessConstants.ROLE_CUSTOMERMANAGERGROUP, auth)) {
        ...
        } else if (userID.equals(ANONYMOUS_USER) && !hasRole(SecuredAccessConstants.ROLE_CUSTOMERGROUP, auth)) {
            setCurrentUser(userService.getAnonymousUser());
        } else if (hasRole(SecuredAccessConstants.ROLE_TRUSTED_CLIENT, auth) || hasRole(SecuredAccessConstants.ROLE_CUSTOMERMANAGERGROUP, auth)) {
            setCurrentUser(userID);
        } else if (hasRole(SecuredAccessConstants.ROLE_CUSTOMERGROUP, auth)) {
    ...
    protected void setCurrentUserForCustomerGroupRole(final String principal, final String userID) {
        ...
        if (userID.equals(CURRENT_USER)) {
            ...
            setCurrentUser(getUserForValidProperty(principal, userID).orElseThrow(() -> new AccessDeniedException("Access is denied")));
        }
    }
