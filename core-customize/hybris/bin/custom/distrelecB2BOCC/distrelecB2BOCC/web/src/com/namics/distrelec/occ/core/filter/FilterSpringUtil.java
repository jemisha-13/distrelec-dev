...
public class FilterSpringUtil {
    ...
    /**
     * Returns the Spring bean with name <code>beanName</code> and of type <code>beanClass</code>. If no bean could be resolved for the
     * specified name, the bean is looked up using type.
     *
     * @param <T>
     *            type of the bean
     * @param httpRequest
    ...
    /**
     * The same as {@link #getSpringBean(HttpServletRequest, String, Class)} but uses ServletContext as the first parameter. It might be used in
     * places, where HttpServletRequest is not available, but ServletContext is.
     */
            ...
            if (beanClass == null) {
                ...
                if (!beansOfType.isEmpty()) {
                    if (beansOfType.size() > 1) {
                        LOG.warn("More than one matching bean found of type " + beanClass.getSimpleName() + ". Returning the first one found.");
                    }
