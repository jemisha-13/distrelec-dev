 ...
 */
package com.namics.distrelec.occ.core.request.mapping.handler;
...
/**
 * Creates {@link RequestMappingInfo} instances from type and method-level {@link RequestMapping @RequestMapping} annotations in
 * {@link Controller @Controller} classes. Additionally it resolve RequestMapping conflicts base on
 * {@link RequestMappingOverride @RequestMappingOverride} annotation. Method with highest priority will be added as handler mapping.
 * CommerceHandlerMapping use also {@link ApiVersion @ApiVersion} annotation to avoid adding request mapping dedicated for different API
 * version of commerce web services
 */
public class CommerceHandlerMapping extends RequestMappingHandlerMapping {
    ...
    /**
     * Scan beans in the ApplicationContext, detect and register handler methods. Additionally this method detect which request mapping was
     * overridden
     */
    @Override
     ...
     *
     * @return the created RequestMappingInfo, or {@code null} if the method does not have a {@code @RequestMapping} annotation or mapping was
     *         overridden by {@code RequestMappingOverride} annotation.
     */
    @Override
    ...
    /**
     * Gets method priority value based on {@code RequestMappingOverride} annotation. Priority value is read from properties files
     * (project.properties, local.properties). If there is no {@code priorityProperty} then {@code <className>.<methodName>.priority} will be
     * used as property name. If there is no value for property with given name, default value = 0 will be returned.
     */
    protected Integer getMethodPriorityValue(final Method method) {
        ...
        if (requestMappingOverride != null) {
            ...
            if (priorityProperty.isEmpty()) {
                priorityProperty = "requestMappingOverride." + method.getDeclaringClass().getName() + "." + method.getName() + ".priority";
            }
    ...
    protected void initOverridenRequestMappings() {
        ...
        final String[] beanNames = this.detectInAncestorContexts ? BeanFactoryUtils.beanNamesForTypeIncludingAncestors(getApplicationContext(), Object.class)
    ...
    protected void detectOverriddenMappings(final Object handler) {
        final Class<?> handlerType = (handler instanceof String) ? getApplicationContext().getType((String) handler) : handler.getClass();
