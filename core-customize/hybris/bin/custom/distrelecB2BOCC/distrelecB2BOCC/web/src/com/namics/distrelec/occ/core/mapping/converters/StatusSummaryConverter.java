...
public class StatusSummaryConverter extends BidirectionalConverter<Map<ProductInfoStatus, Integer>, List<StatusSummaryWsDTO>> {
    @Override
    public Map<ProductInfoStatus, Integer> convertFrom(final List<StatusSummaryWsDTO> statusSummaryList, final Type<Map<ProductInfoStatus, Integer>> type,
                                                       final MappingContext mappingContext) {
        if (statusSummaryList != null) {
            return statusSummaryList.stream().collect(Collectors.toMap(entry -> valueOf(entry.getStatus()), entry -> entry.getNumberOfIssues()));
        }
        return Collections.emptyMap();
    }

    @Override
    public List<StatusSummaryWsDTO> convertTo(final Map<ProductInfoStatus, Integer> productInfoStatusMap, final Type<List<StatusSummaryWsDTO>> type,
                                              final MappingContext mappingContext) {
