...
public class CancellationRequestEntryInputListDTOValidator implements Validator {
    ...
    public void validate(final Object target, final Errors errors) {
        ...
        final List<CancellationRequestEntryInputWsDTO> cancellationRequestEntryInputs = cancellationRequestEntryInputList.getCancellationRequestEntryInputs();
        if (CollectionUtils.isEmpty(cancellationRequestEntryInputs)) {
            ...
            IntStream.range(0, cancellationRequestEntryInputs.size()).filter(i -> Objects.isNull(cancellationRequestEntryInputs.get(i).getOrderEntryNumber()))
                     .forEach(i -> errors.rejectValue(String.format("cancellationRequestEntryInputs[%d].orderEntryNumber", i), FIELD_REQUIRED_MESSAGE_ID));

            IntStream.range(0, cancellationRequestEntryInputs.size()).filter(i -> Objects.isNull(cancellationRequestEntryInputs.get(i).getQuantity()))
                     .forEach(i -> errors.rejectValue(String.format("cancellationRequestEntryInputs[%d].quantity", i), FIELD_REQUIRED_MESSAGE_ID));
