...
public class CustomerGroupsController extends BaseController {
    ...
    @ApiBaseSiteIdParam
    public void createCustomerGroup(@Parameter(description = "Id of new customer group.", required = true) @RequestParam final String groupId,
    ...
    @Secured("ROLE_CUSTOMERMANAGERGROUP")
    @Operation(operationId = "createCustomerGroup", summary = "Creates a new customer group.", description = "Creates a new customer group that is a direct subgroup of a customergroup.\n\nTo try out the methods of the Customer "
                                                                                                     + "Groups controller, you must authorize a user who belongs to the “customermanagergroup”.")
    @ApiBaseSiteIdParam
    public void createCustomerGroup(@Parameter(description = "User group object with id and name.", required = true) @RequestBody final UserGroupWsDTO userGroup) {
    ...
    @ApiBaseSiteIdParam
    public void updateCustomerGroupWithUsers(@Parameter(description = "Group identifier.", required = true) @PathVariable final String groupId,
    ...
    @Secured("ROLE_CUSTOMERMANAGERGROUP")
    @ResponseStatus(value = HttpStatus.OK)
    @Operation(operationId = "updateCustomerGroupWithUsers", summary = "Assigns user(s) to a customer group.", description = "Assigns user(s) to a customer group.\n\nTo try out the methods of the Customer Groups controller, you must "
    ...
    @ApiBaseSiteIdParam
    public void replaceUserListForCustomerGroup(@Parameter(description = "Group identifier.", required = true) @PathVariable final String groupId,
    ...
    protected void setUserListForCustomerGroupInternal(final String groupId, final List<String> members) {
        final UserGroupData userGroup = customerGroupFacade.getCustomerGroup(groupId, Collections.singleton(UserGroupOption.MEMBERS));
    ...
    @RequestMapping(value = "/{groupId}/members", method = RequestMethod.PUT, consumes = { MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE })
    @Secured("ROLE_CUSTOMERMANAGERGROUP")
    @ResponseStatus(value = HttpStatus.OK)
    @Operation(operationId = "replaceUsersForCustomerGroup", summary = "Sets members for a user group.", description = "Sets members for a user group. The list of existing members is overwritten with a new one.\n\nTo try out the methods "
                                                                                                               + "of the Customer Groups controller, you must authorize a user who belongs to the “customermanagergroup”.")
    @ApiBaseSiteIdParam
    public void replaceUsersForCustomerGroup(@Parameter(description = "Group identifier.", required = true) @PathVariable final String groupId,
    ...
    @Secured("ROLE_CUSTOMERMANAGERGROUP")
    @ResponseStatus(value = HttpStatus.OK)
    @Operation(operationId = "removeUsersFromCustomerGroup", summary = "Deletes a user from a customer group.", description = "Deletes user from a customer group.\n\nTo try out the methods of the Customer Groups controller, you must "
                                                                                                                      + "authorize a user who belongs to the “customermanagergroup”.")
    @ApiBaseSiteIdParam
    public void removeUsersFromCustomerGroup(@Parameter(description = "Group identifier.", required = true) @PathVariable final String groupId,
    ...
    @ResponseStatus(value = HttpStatus.OK)
    @ResponseBody
    @Operation(operationId = "getCustomerGroups", summary = "Get all subgroups of a customergroup.", description = "Returns all customer groups that are direct subgroups of a customergroup.\n\nTo try out the methods of the "
    ...
    public UserGroupListWsDTO getCustomerGroups(
                                                ...
                                                @Parameter(description = "Number of customer group returned in one page.") @RequestParam(defaultValue = "0") final int pageSize,
                                                @ApiFieldsParam(defaultValue = BASIC_FIELD_SET) @RequestParam(defaultValue = BASIC_FIELD_SET) final String fields) {
        final PageOption pageOption = PageOption.createForPageNumberAndPageSize(currentPage, getPageSizeOrDefault(pageSize));
    ...
    @ResponseStatus(value = HttpStatus.OK)
    @ResponseBody
    @Operation(operationId = "getCustomerGroup", summary = "Get a specific customer group.", description = "Returns a customer group with a specific groupId.\n\nTo try out the methods of the Customer Groups controller, you "
                                                                                                   + "must authorize a user who belongs to the “customermanagergroup”.")
    @ApiBaseSiteIdParam
    public UserGroupWsDTO getCustomerGroup(@Parameter(description = "Group identifier.", required = true) @PathVariable final String groupId,
                                           @ApiFieldsParam(defaultValue = BASIC_FIELD_SET) @RequestParam(defaultValue = BASIC_FIELD_SET) final String fields) {
    ...
    protected String createOperationErrorMessage(final String operationMessage, final String groupId, final String userId) {
        return String.join("", operationMessage, sanitize(groupId), ". User '", sanitize(userId), "' doesn't exist or you have no privileges");
    }
