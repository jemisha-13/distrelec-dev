 ...
 */
package com.namics.distrelec.occ.core.auth;
...
/**
 * This filter should be used after spring security filters and it is responsible for setting current authentication as guest when user
 * decided to do the checkout as a guest. During the guest checkout the userService gets current user as 'anonymous', but cartService
 * returns dedicated user.
 */
public class GuestRoleFilter extends OncePerRequestFilter {
    ...
    protected static boolean canProcessAuthentication(final CustomerModel customerModel) {
        ...
        return currentAuth == null || !currentAuth.getClass().equals(GuestAuthenticationToken.class) || !customerModel.getUid().equals(currentAuth.getName());
    }
    ...
    protected Optional<CustomerModel> getSessionCartUser() {
        if (cartService.hasSessionCart()) {
            return Optional.ofNullable(cartService.getSessionCart().getUser()).filter(user -> CustomerModel.class.isAssignableFrom(user.getClass()))//
