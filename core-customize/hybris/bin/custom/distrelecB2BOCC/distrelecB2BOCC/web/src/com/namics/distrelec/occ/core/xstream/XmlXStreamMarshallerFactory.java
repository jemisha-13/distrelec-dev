...
/**
 * Factory for creating a {@link XStreamMarshaller} with given {@link XStream}, registered available {@link TypeAliasMapping} and
 * {@link TypeConverterMapping} instances as customization of the {@link XStreamMarshaller} .
 */
public class XmlXStreamMarshallerFactory implements FactoryBean, ApplicationContextAware, InitializingBean {
    ...
    protected XStreamMarshaller getObjectInternal() {
        ...
        // we use here BeanFactoryUtils.beansOfTypeIncludingAncestors lookup for assuring the lookup for the 'beans of type' in the parent spring
        // context succeeds
        final Map<String, TypeAliasMapping> allTypeAliases = BeanFactoryUtils.beansOfTypeIncludingAncestors(ctx, TypeAliasMapping.class);
        setAliases(marshaller, allTypeAliases.values());

        final Map<String, TypeConverterMapping> allTypeConverter = BeanFactoryUtils.beansOfTypeIncludingAncestors(ctx, TypeConverterMapping.class);
        setConverters(marshaller, allTypeConverter.values());

        final Map<String, AttributeOmitMapping> allOmitersConverter = BeanFactoryUtils.beansOfTypeIncludingAncestors(ctx, AttributeOmitMapping.class);
    ...
    protected void configureXmlMarshaller(final XStreamMarshaller marshaller) {
        final Map<String, ImplicitCollection> allImplicitCollections = BeanFactoryUtils.beansOfTypeIncludingAncestors(ctx, ImplicitCollection.class);
    ...
    protected void setImplicitCollections(final XStreamMarshaller marshaller, final Collection<ImplicitCollection> values) {
        for (final ImplicitCollection implicit : values) {
            marshaller.getXStream().addImplicitCollection(implicit.getOwnerType(), implicit.getFieldName(), implicit.getItemFieldName(),
    ...
    protected void setAttributeConverterInternal(final XStreamMarshaller marshaller, final AttributeConverterMapping converterMapping) {
    ...
    protected void setAttributeAliasInternal(final XStreamMarshaller marshaller, final TypeAliasMapping alias) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("registering attribute alias " + attrAlias.getAlias() + " , " + attrAlias.getAttributeName() + "." + attrAlias.getAliasedClass());
        }
    ...
    protected void setFieldAliasInternal(final XStreamMarshaller marshaller, final TypeAliasMapping alias) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("registering field alias " + attrAlias.getAlias() + " , " + attrAlias.getFieldName() + "." + attrAlias.getAliasedClass());
        }
