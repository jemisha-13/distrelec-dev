 ...
 */
package com.namics.distrelec.occ.core.errors.converters;
...
public class CartModificationDataErrorConverterTest {
    ...
    public void setUp() {
        ...
        lenient().when(messageSource.getMessage(eq(NO_STOCK_MESSAGE), any(Object[].class), nullable(Locale.class))).thenReturn(NO_STOCK_MESSAGE);
        lenient().when(messageSource.getMessage(eq(LOW_STOCK_MESSAGE), any(Object[].class), nullable(Locale.class))).thenReturn(LOW_STOCK_MESSAGE);
        given(messageSource.getMessage(eq(NO_STOCK_MESSAGE), any(Object[].class), anyString(), nullable(Locale.class))).willReturn(NO_STOCK_MESSAGE);
        given(messageSource.getMessage(eq(LOW_STOCK_MESSAGE), any(Object[].class), anyString(), nullable(Locale.class))).willReturn(LOW_STOCK_MESSAGE);
        given(messageSource.getMessage(eq(OTHER_MESSAGE), any(Object[].class), anyString(), nullable(Locale.class))).willReturn(OTHER_MESSAGE);
    }
    ...
    public void testConvertWhenOutOfStockWithoutEntryNumber() {
        ...
        final ErrorWsDTO error = result.get(0);
        assertEquals("Error type should be insufficient stock error", CartModificationDataErrorConverter.TYPE, error.getType());
        assertEquals("Subject type should be product", CartModificationDataErrorConverter.SUBJECT_TYPE_PRODUCT, error.getSubjectType());
    ...
    public void testConvertWhenOtherError() {
        ...
        assertEquals("Error result should exist", 1, result.size());
        final ErrorWsDTO error = result.get(0);
        assertEquals("Error type should be generic validation error", CartModificationDataErrorConverter.TYPE_OTHER, error.getType());
        assertEquals("Subject type should be product", CartModificationDataErrorConverter.SUBJECT_TYPE_PRODUCT, error.getSubjectType());
        assertEquals("Subject should be product code", PRODUCT_CODE, error.getSubject());
    ...
    public void testPopulateSubjectFromEntryNumber() {
        ...
        assertEquals("Subject type should be entry if entry number exist", CartModificationDataErrorConverter.SUBJECT_TYPE_ENTRY, errorWsDTO.getSubjectType());
    ...
    public void testPopulateSubjectFromProductCode() {
        ...
        assertEquals("Subject type should be product if product exist", CartModificationDataErrorConverter.SUBJECT_TYPE_PRODUCT, errorWsDTO.getSubjectType());
    }
