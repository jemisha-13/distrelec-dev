 ...
 */
package com.namics.distrelec.occ.core.queues.impl;

import com.namics.distrelec.occ.core.queues.data.OrderStatusUpdateElementData;
...
public class OrderStatusUpdateQueueTest {
    ...
    public void setUp() throws ParseException {
        ...
        // 27 hours ago
        Mockito.doReturn(Long.valueOf((new Date(tick.getTime() - 60 * 60 * 27 * 1000).getTime()))).when(orderStatusUpdateQueue).getTimeKey(orderElement0);

        orderElement1 = new OrderStatusUpdateElementData();
        orderElement1.setCode(ORDER1_CODE);
        // 26 hours ago
        Mockito.doReturn(Long.valueOf((new Date(tick.getTime() - 60 * 60 * 26 * 1000).getTime()))).when(orderStatusUpdateQueue).getTimeKey(orderElement1);

        orderElement2 = new OrderStatusUpdateElementData();
        orderElement2.setCode(ORDER2_CODE);
        // 25 hours ago
        Mockito.doReturn(Long.valueOf((new Date(tick.getTime() - 60 * 60 * 25 * 1000).getTime()))).when(orderStatusUpdateQueue).getTimeKey(orderElement2);

        orderElement3 = new OrderStatusUpdateElementData();
        orderElement3.setCode(ORDER3_CODE);
        // 10 hours ago
        Mockito.doReturn(Long.valueOf((new Date(tick.getTime() - 60 * 60 * 10 * 1000).getTime()))).when(orderStatusUpdateQueue).getTimeKey(orderElement3);
