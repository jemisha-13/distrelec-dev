...
public class ProductExpressUpdateQueueTest {
    ...
    public void setUp() throws ParseException {
        ...
        // 27 hours ago
        Mockito.doReturn(Long.valueOf((new Date(tick.getTime() - 60 * 60 * 27 * 1000).getTime()))).when(productExpressUpdateQueue).getTimeKey(productElement0);

        productElement1 = new ProductExpressUpdateElementData();
        productElement1.setCode(PRODUCT1_CODE);
        // 26 hours ago
        Mockito.doReturn(Long.valueOf((new Date(tick.getTime() - 60 * 60 * 26 * 1000).getTime()))).when(productExpressUpdateQueue).getTimeKey(productElement1);

        productElement2 = new ProductExpressUpdateElementData();
        productElement2.setCode(PRODUCT2_CODE);
        // 25 hours ago
        Mockito.doReturn(Long.valueOf((new Date(tick.getTime() - 60 * 60 * 25 * 1000).getTime()))).when(productExpressUpdateQueue).getTimeKey(productElement2);

        productElement3 = new ProductExpressUpdateElementData();
        productElement3.setCode(PRODUCT3_CODE);
        // 10 hours ago
        Mockito.doReturn(Long.valueOf((new Date(tick.getTime() - 60 * 60 * 10 * 1000).getTime()))).when(productExpressUpdateQueue).getTimeKey(productElement3);
