...
class CartEntriesTest extends AbstractCartTest {
    ...
    def "Customer tries to add product without required code parameter when request: #requestFormat and response: #responseFormat"() {
        ...
        where:
        requestFormat | responseFormat | postBody                                                                                                       | errorType                             | errorMessagePattern
        URLENC        | XML            | ['qty': 1]                                                                                                     | 'MissingServletRequestParameterError' | /Required \w+ parameter 'code'.* is not present/
        URLENC        | JSON           | ['qty': 1]                                                                                                     | 'MissingServletRequestParameterError' | /Required \w+ parameter 'code'.* is not present/
    ...
    def "Customer tries to replace entry without required quantity parameter when request: #requestFormat and response: #responseFormat"() {
        ...
        where:
        requestFormat | responseFormat | postBody                                                                                                                                           | errorType                             | errorMessagePattern
        URLENC        | XML            | ['pickupStore': STORE_NAKANO]                                                                                                                      | 'MissingServletRequestParameterError' | /Required \w+ parameter 'qty'.* is not present/
        URLENC        | JSON           | ['pickupStore': STORE_NAKANO]                                                                                                                      | 'MissingServletRequestParameterError' | /Required \w+ parameter 'qty'.* is not present/
