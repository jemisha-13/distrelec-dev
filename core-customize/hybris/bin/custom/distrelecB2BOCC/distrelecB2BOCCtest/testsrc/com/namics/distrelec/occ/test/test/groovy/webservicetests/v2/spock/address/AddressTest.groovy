 ...
 *
 */
package com.namics.distrelec.occ.test.test.groovy.webservicetests.v2.spock.address

import com.namics.distrelec.occ.test.test.groovy.webservicetests.v2.spock.users.AbstractUserTest
import de.hybris.bootstrap.annotations.ManualTest
import groovyx.net.http.HttpResponseDecorator
import spock.lang.IgnoreIf
import spock.lang.Unroll

import static com.namics.distrelec.occ.test.setup.TestSetupUtils.isExtensionPresent
...
class AddressTest extends AbstractUserTest {
    ...
    private static final String BAD_EMAIL1 = 'invalid'
    private static final String BAD_EMAIL2 = 'test@'
    private static final String BAD_EMAIL3 = '@sap.com'
    private static final String BAD_EMAIL4 = 'test@asd.asd'
    ...
    def "Customer creates address with district, cellphone and email when request: #requestFormat and response: #responseFormat"() {
        //This is happy path
        given: "a registered and logged in customer"
        authorizeTrustedClient(restClient)
        def customer = registerCustomer(restClient, responseFormat)
        ...
        where:
        requestFormat | responseFormat | postBody
        URLENC        | JSON           | ['titleCode': CUSTOMER_TITLE_CODE, 'town': CUSTOMER_ADDRESS_TOWN, 'line1': CUSTOMER_ADDRESS_LINE1, 'postalCode': CUSTOMER_ADDRESS_POSTAL_CODE, 'country.isocode': CUSTOMER_ADDRESS_COUNTRY_ISO_CODE, 'firstName': CUSTOMER_FIRST_NAME, 'lastName': CUSTOMER_LAST_NAME, 'district': CUSTOMER_ADDRESS_DISTRICT, 'cellphone': CUSTOMER_ADDRESS_CELLPHONE, 'email': CUSTOMER_EMAIL]
        ...
        XML           | XML            | "<address><titleCode>${CUSTOMER_TITLE_CODE}</titleCode><town>${CUSTOMER_ADDRESS_TOWN}</town><line1>${CUSTOMER_ADDRESS_LINE1}</line1><postalCode>${CUSTOMER_ADDRESS_POSTAL_CODE}</postalCode><country><isocode>${CUSTOMER_ADDRESS_COUNTRY_ISO_CODE}</isocode></country><firstName>${CUSTOMER_FIRST_NAME}</firstName><lastName>${CUSTOMER_LAST_NAME}</lastName><district>${CUSTOMER_ADDRESS_DISTRICT}</district><cellphone>${CUSTOMER_ADDRESS_CELLPHONE}</cellphone><email>${CUSTOMER_EMAIL}</email></address>"
    }
    ...
    def "Customer gets single address : #format"() {
        ...
        def customer = registerCustomer(restClient, format)
        authorizeCustomer(restClient, customer)
        def address = createAddressWithOptions(restClient, customer, format: format, withBillingEmail: true)
    ...
    def "Customer gets single address when parameter fields euqals DEFAULT, checking presence of titleCode and email : #format"() {
        given: "a registered and logged in customer with already existing address"
        authorizeTrustedClient(restClient)
        def customer = registerCustomer(restClient, format)
        authorizeCustomer(restClient, customer)
    ...
    def "Customer edits his own address when request: #requestFormat and response: #responseFormat"() {
        ...
        def customer = registerCustomer(restClient, responseFormat)
        authorizeCustomer(restClient, customer)
        def address = createAddressWithOptions(restClient, customer, format: responseFormat, withBillingEmail: true)
    ...
    def "Address gets rejected when incorrect email is sent when request: #requestFormat and response: #responseFormat"() {
        given: "a logged in customer"
        def customer = registerAndAuthorizeCustomer(restClient, responseFormat)
        ...
        then: "the address is rejected and proper error is returned"
        with(response) {
            if (isNotEmpty(data) && isNotEmpty(data.errors)) println(data)
            status == SC_BAD_REQUEST
            data.errors.size() > 0
        ...
        where:
        requestFormat | responseFormat | postBody
        URLENC        | JSON           | ['titleCode': CUSTOMER_TITLE_CODE, 'town': CUSTOMER_ADDRESS_TOWN, 'line1': CUSTOMER_ADDRESS_LINE1, 'postalCode': CUSTOMER_ADDRESS_POSTAL_CODE, 'country.isocode': CUSTOMER_ADDRESS_COUNTRY_ISO_CODE, 'firstName': CUSTOMER_FIRST_NAME, 'lastName': CUSTOMER_LAST_NAME, 'district': CUSTOMER_ADDRESS_DISTRICT, 'cellphone': CUSTOMER_ADDRESS_CELLPHONE, 'email': BAD_EMAIL1]
        URLENC        | XML            | ['titleCode': CUSTOMER_TITLE_CODE, 'town': CUSTOMER_ADDRESS_TOWN, 'line1': CUSTOMER_ADDRESS_LINE1, 'postalCode': CUSTOMER_ADDRESS_POSTAL_CODE, 'country.isocode': CUSTOMER_ADDRESS_COUNTRY_ISO_CODE, 'firstName': CUSTOMER_FIRST_NAME, 'lastName': CUSTOMER_LAST_NAME, 'district': CUSTOMER_ADDRESS_DISTRICT, 'cellphone': CUSTOMER_ADDRESS_CELLPHONE, 'email': BAD_EMAIL2]
        JSON          | JSON           | "{\"titleCode\": \"${CUSTOMER_TITLE_CODE}\", \"town\": \"${CUSTOMER_ADDRESS_TOWN}\", \"line1\": \"${CUSTOMER_ADDRESS_LINE1}\", \"postalCode\": \"${CUSTOMER_ADDRESS_POSTAL_CODE}\", \"country\": { \"isocode\": \"${CUSTOMER_ADDRESS_COUNTRY_ISO_CODE}\" }, \"firstName\": \"${CUSTOMER_FIRST_NAME}\", \"lastName\": \"${CUSTOMER_LAST_NAME}\", \"district\": \"${CUSTOMER_ADDRESS_DISTRICT}\", \"cellphone\": \"${CUSTOMER_ADDRESS_CELLPHONE}\", \"email\": \"${BAD_EMAIL3}\"  }"
        XML           | XML            | "<address><titleCode>${CUSTOMER_TITLE_CODE}</titleCode><town>${CUSTOMER_ADDRESS_TOWN}</town><line1>${CUSTOMER_ADDRESS_LINE1}</line1><postalCode>${CUSTOMER_ADDRESS_POSTAL_CODE}</postalCode><country><isocode>${CUSTOMER_ADDRESS_COUNTRY_ISO_CODE}</isocode></country><firstName>${CUSTOMER_FIRST_NAME}</firstName><lastName>${CUSTOMER_LAST_NAME}</lastName><district>${CUSTOMER_ADDRESS_DISTRICT}</district><cellphone>${CUSTOMER_ADDRESS_CELLPHONE}</cellphone><email>${BAD_EMAIL4}</email></address>"
    }

    /**
     * Below @IgnoreIf should be removed after fix of DefaultCisAddressVerificationService in cisavs extension
     */
    @IgnoreIf({ isExtensionPresent("cisavs") })
