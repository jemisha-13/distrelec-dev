...
public class TestSetupUtils {
    private static final String[] EXTENSIONS_TO_START = new String[] { YcommercewebservicesConstants.EXTENSIONNAME, OAuth2Constants.EXTENSIONNAME };
    ...
    private static void setupCommerce(final BiConsumer<DistrelecB2BOCCTestSetup, SystemSetupContext> triggerEqualsValidator) {
        final DistrelecB2BOCCTestSetup setup = getCommerceWebServicesTestSetup();
    ...
    private static void initPaymentCommandFactory() {
        final DefaultCommandFactoryRegistryImpl commandFactoryReg = Registry.getApplicationContext().getBean("commandFactoryRegistry",
        ...
        // set command factory registry because some tests change it and not restore
        // (QuotesProcessIntegrationTest,AnonymousCheckoutIntegrationTest,DefaultExpressCheckoutIntegrationCheckoutTest)
    ...
    private static DefaultCommandFactoryImpl createPaymentCommandFactory() {
        ...
        commands.put(de.hybris.platform.payment.commands.IsApplicableCommand.class, new de.hybris.platform.payment.commands.impl.IsApplicableMockCommand());
        commands.put(de.hybris.platform.payment.commands.AuthorizationCommand.class, new de.hybris.platform.payment.commands.impl.AuthorizationMockCommand());
        ...
        commands.put(de.hybris.platform.payment.commands.CaptureCommand.class, new de.hybris.platform.payment.commands.impl.CaptureMockCommand());
        commands.put(de.hybris.platform.payment.commands.PartialCaptureCommand.class, new de.hybris.platform.payment.commands.impl.PartialCaptureMockCommand());
        ...
        commands.put(de.hybris.platform.payment.commands.VoidCommand.class, new de.hybris.platform.payment.commands.impl.VoidMockCommand());
        commands.put(de.hybris.platform.payment.commands.FollowOnRefundCommand.class, new de.hybris.platform.payment.commands.impl.FollowOnRefundMockCommand());
    ...
    private static DistrelecB2BOCCTestSetup getCommerceWebServicesTestSetup() {
        return Registry.getApplicationContext().getBean("yCommerceWebServicesTestSetup", DistrelecB2BOCCTestSetup.class);
    }

    private static SystemSetupContext createSystemSetupContext() {
        final Map<String, String[]> params = new HashMap<>();
        params.put("init", new String[] { "Go" });
        params.put("distrelecB2BOCCtest_sample", new String[] { "true" });
        params.put("lucenesearch_rebuild", new String[] { "true" });
        params.put("lucenesearch_update.index.configuration", new String[] { "true" });

        return new SystemSetupContext(params, Type.ALL, Process.ALL, "distrelecB2BOCCtest");
    }
    ...
    public static void afterInitializeTestSystem() {
        ...
        if (ctx.containsBean(ITEM_CREATION_LIFECYCLE_LISTENER)) {
            final ItemCreationLifecycleListener itemCreationLifecycleListener = (ItemCreationLifecycleListener) ctx.getBean(ITEM_CREATION_LIFECYCLE_LISTENER);
    ...
    public static boolean isServerStarted() {
        final EmbeddedServerController controller = Registry.getApplicationContext().getBean("embeddedServerController", EmbeddedServerController.class);
    ...
    public static void startServer(final String[] ext) {
        if (!Config.getBoolean("distrelecB2BOCCtest.embedded.server.enabled", true)) {
