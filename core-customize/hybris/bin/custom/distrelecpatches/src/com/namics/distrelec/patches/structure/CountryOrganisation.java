...
public enum CountryOrganisation implements ImportOrganisationUnit<CountryOrganisation, ShopOrganisation> {
    ...
    CountryOrganisation(final String code, final String name, final ImportLanguage[] languages, final StructureState structureState) {
        this.code = code;
        this.name = name;
        this.languages = Arrays.asList(languages);
    ...
    public void setParent(final ShopOrganisation parent) {
        ...
        if (parentStructureState.isAfter(this.structureState)) {
            this.structureState = parentStructureState;
            LOG.warn("CountryOrganization with name " + this.getName() + " have structureState set that is before store where this country belongs " +
                       "too");
        }
    }
