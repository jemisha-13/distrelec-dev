package distrelecpatches.releases.releaser1_16.patch_patch_32448.global
...
ThreadFactory tenantAwareThreadFactory = applicationContext.getBean(TenantAwareThreadFactory.class)
        ...
        List<Future> futures = batchSubLists.stream().map { pkList ->
            return dbUpdateExecutor.submit {
                StringBuilder updateQueryText = new StringBuilder("""
                    update distsalesorgproduct p
                    set p.p_endoflifedate = case
                            when p.p_lastmodifiederp is not null then p.p_lastmodifiederp
                            when p.modifiedts is not null then p.modifiedts
                            else null
                        end
                    where p.pk in (
                 """)
