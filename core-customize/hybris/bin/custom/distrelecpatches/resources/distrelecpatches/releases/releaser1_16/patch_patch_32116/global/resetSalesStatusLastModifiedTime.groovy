package distrelecpatches.releases.releaser1_16.patch_patch_32116.global
...
@Field Logger LOG = LoggerFactory.getLogger("SALES_STATUS_ERP_TIME_RESET_SCRIPT")

@Field int batchSize = 100000

def getSelectQuery() {
    if (Config.isSQLServerUsed()) {
        return "SELECT TOP $batchSize pk FROM distsalesorgproduct WHERE p_lastmodifiederp IS NOT NULL"
    } else {
        return "SELECT pk FROM distsalesorgproduct WHERE p_lastmodifiederp IS NOT NULL ${getLimitQuery()}"
    }
}
...
def getTotalCount() {
    Statement statement = connection.prepareStatement("SELECT COUNT(*) FROM distsalesorgproduct WHERE p_lastmodifiederp IS NOT NULL")
    try {
...
def updateNextBatch() {
    String query = """UPDATE distsalesorgproduct SET p_lastmodifiederp = NULL WHERE pk IN (${getSelectQuery()})"""
