...
import com.namics.distrelec.b2b.test.services.AccountManagerApproveScheduleService;
...
public class B2BAcceleratorTestOrderData {
    ...
    /**
     * Main method Create 12 Orders for the users. Orders have unique identifiable PONumbers. If this method is called more than once, the
     * PONumbers will match and new orders will not be placed.
     */
    public void createSampleOrders() {
        LOG.info("Creating Sample Orders");

        placeOrderWithInCustomerPermissions(Namb2bacceleratorTestConstants.PAY_USER_UID, "ORDER_SENT_NOTIFICATION_SENT paid", false);

        placeOrderRequiringApproval(Namb2bacceleratorTestConstants.HISTORICAL_USER_UID, "PENDING_APPROVAL");
        placeOrderForQuoteNegotiation(Namb2bacceleratorTestConstants.HISTORICAL_USER_UID, "PENDING_QUOTE");
        placeOrderAndApproveByB2BApprover(Namb2bacceleratorTestConstants.HISTORICAL_USER_UID, "ORDER_SENT_NOTIFICATION_SENT Approved");
        placeOrderWithInCustomerPermissions(Namb2bacceleratorTestConstants.HISTORICAL_USER_UID, "ORDER_SENT_NOTIFICATION_SENT costcenter", true);

        placeReplenishmentOrderInCancelledState(Namb2bacceleratorTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ENDED");
        placeReplenishmentOrderInActiveState(Namb2bacceleratorTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ONGOING");
        placeReplenishmentOrderScheduleForFuture(Namb2bacceleratorTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_FUTURE");

        placeOrderQuoteRequestWithMerchantResponse(Namb2bacceleratorTestConstants.QUOTE_USER_UID, "APPROVED_QUOTE");
        placeOrderQuoteRequestWithMerchantResponse(Namb2bacceleratorTestConstants.QUOTE_USER_UID, "REJECTED_QUOTE");
        placeOrderExceedingCreditLimit(Namb2bacceleratorTestConstants.QUOTE_USER_UID, "PENDING_MERCHANT_APPROVAL");
        placeOrderExceedingCreditAndRejected(Namb2bacceleratorTestConstants.QUOTE_USER_UID, "REJECTED_BY_MERCHANT");

        LOG.info("Finished Creating Sample Orders");
    }

    public void createPaymentInfos() {
        createPaymentInfo(Namb2bacceleratorTestConstants.PAY_USER_UID, "USD", createVisaCardInfo(), createUsBillingInfo());
        createPaymentInfo(Namb2bacceleratorTestConstants.PAY_USER_UID, "USD", createMasterCardInfo(), createGermanyBillingInfo());
    }
    ...
    protected String placeReplenishmentOrderInCancelledState(final String userID, final String purchaseOrderNumber) {
        ...
        try {
            ...
            code = scheduleOrder(userID, products, createUsAddressData(), purchaseOrderNumber, Namb2bacceleratorTestConstants.STANDARD_COSTCENTER, null,
                    triggerData).getJobCode();
            if (code != null) {
    ...
    /**
     * Create schedule order to begin in the past so it will be picked up immediately as it's overdue. Run an additional time manually.
     */
    ...
    protected String placeReplenishmentOrderScheduleForFuture(final String userID, final String purchaseOrderNumber) {
        LOG.debug(String.format("******************Starting Attempt to setup and create order for PONumber: %s for " + "%s ", purchaseOrderNumber, userID));
        String code = null;
        try {
            final TriggerData triggerData = new TriggerData();
            triggerData.setActivationTime(DateUtils.addYears(new Date(), 20));// Twenty Years From Now
    ...
    protected String placeOrderWithInCustomerPermissions(final String userID, final String purchaseOrderNumber, final boolean useCostCenter) {
        LOG.debug(String.format("******************Starting Attempt to setup and create order for PONumber: %s for " + "%s ", purchaseOrderNumber, userID));
        ...
        try {
            ...
            final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
                    useCostCenter ? Namb2bacceleratorTestConstants.STANDARD_COSTCENTER : null, null);
    ...
    protected String placeOrderRequiringApproval(final String userID, final String purchaseOrderNumber) {
        ...
        try {
            ...
            final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
                    Namb2bacceleratorTestConstants.STANDARD_COSTCENTER, null);
    ...
    protected String placeOrderForQuoteNegotiation(final String userID, final String purchaseOrderNumber) {
        ...
        try {
            ...
            final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
                    Namb2bacceleratorTestConstants.STANDARD_COSTCENTER, "Please approve request");
    ...
    protected String placeOrderExceedingCreditAndRejected(final String userID, final String purchaseOrderNumber) {
        ...
        try {
            ...
            final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
                    Namb2bacceleratorTestConstants.LOWLIMIT_COSTCENTER, null);
    ...
    protected void createPaymentInfo(final String customerUid, final String currencyIso, final CardInfo cardInfo, final BillingInfo billingInfo) {
        // Lookup the site
        final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(Namb2bacceleratorTestConstants.POWERTOOLS_SITE);
        // Lookup the customer
        ...
        getImpersonationService().executeInContext(ctx, new ImpersonationService.Executor<Object, ImpersonationService.Nothing>() {
            ...
            public Object execute() throws ImpersonationService.Nothing {
                ...
                if (!containsCardInfo(storedCards, cardInfo)) {
                    ...
                    final CreditCardPaymentInfoModel creditCardPaymentInfoModel = getCustomerAccountService().createPaymentSubscription(customer, cardInfo,
                            billingInfo, customerTitleCode, getPaymentProvider(), true);
    ...
    protected OrderData placeOrder(final String customerUid, final Map<String, Long> products, final AddressData deliveryAddress,
        ...
        final ImpersonationContext ctx = new ImpersonationContext();
        ctx.setSite(getCmsAdminSiteService().getSiteForId(Namb2bacceleratorTestConstants.POWERTOOLS_SITE));
        ctx.setUser(customer);
        ctx.setCurrency(i18nService.getCurrency(CURRENCY_ISO_CODE));

        return getImpersonationService().executeInContext(ctx, new ImpersonationService.Executor<OrderData, ImpersonationService.Nothing>() {
            @Override
            public OrderData execute() throws ImpersonationService.Nothing {

                OrderData orderData = null;

                // Check if the order already exists by PurchaseOrderNumber
                if (prepareANewSessionCartForCheckout(products, deliveryAddress, purchaseOrderNumber, costCenterCode, quotePetition, customer)) {
                    try {
    ...
    protected ScheduledCartData scheduleOrder(final String customerUid, final Map<String, Long> products, final AddressData deliveryAddress,
            final String purchaseOrderNumber, final String costCenterCode, final String quotePetition, final TriggerData triggerData) {
        // Impersonate site and customer
        final B2BCustomerModel customer = getUserService().getUserForUID(customerUid.toLowerCase(), B2BCustomerModel.class);
        ...
        return (ScheduledCartData) getImpersonationService().executeInContext(ctx, new ImpersonationService.Executor<Object, ImpersonationService.Nothing>() {
            @Override
            public Object execute() throws ImpersonationService.Nothing {

                ScheduledCartData scheduledCartData = new ScheduledCartData();// null object
                // Check if the order already exists by PurchaseOrderNumber
    ...
    protected boolean prepareANewSessionCartForCheckout(final Map<String, Long> products, final AddressData deliveryAddress, final String purchaseOrderNumber,
            final String costCenterCode, final String quotePetition, final B2BCustomerModel customer) {
        LOG.info(String.format("Creating order for [%s] for site [%s]", customer.getUid(), Namb2bacceleratorTestConstants.POWERTOOLS_SITE));

        final List<OrderModel> orderList = getCustomerAccountService().getOrderList(customer, getBaseStoreSelectorStrategy().getCurrentBaseStore(), null);

        if (containsOrder(orderList, purchaseOrderNumber)) {
            LOG.info(String.format("Not creating order because purchaseOrderNumber %s for [%s] for site [%s] already exists. ", purchaseOrderNumber,
                    customer.getUid(), Namb2bacceleratorTestConstants.POWERTOOLS_SITE));
            ...
            if (costCenterCode == null) {
                ...
                // getB2bCheckoutFacade().setPaymentDetailsIfAvailable();
    ...
    public void sendToAccountManagerToRejectIfPONumContainsReject(final String orderCode, final String customerID) {
        accountManagerApproveScheduleService.schedleAccountManagerJobToApproveReject(orderCode, customerID);

    }

    protected B2BOrderApprovalData b2bApproverApproveThisOrder(final String orderCode) {
        final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(Namb2bacceleratorTestConstants.POWERTOOLS_SITE);
        final B2BCustomerModel customer = getUserService().getUserForUID(Namb2bacceleratorTestConstants.APPROVER_UID.toLowerCase(), B2BCustomerModel.class);
    ...
    private B2BOrderApprovalData adjustOrderApproveDecision(final String orderCode, final String decision, final String comment) {
    ...
    protected boolean matchesCardInfo(final CreditCardPaymentInfoModel storedCard, final CardInfo cardInfo) {
        return (storedCard.getType().equals(cardInfo.getCardType()) && StringUtils.equals(storedCard.getCcOwner(), cardInfo.getCardHolderFullName()));
    }
    ...
    public void setAccountManagerApproveScheduleService(final AccountManagerApproveScheduleService accountManagerApproveScheduleService) {
