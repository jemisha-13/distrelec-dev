...
incrementalDateTime = '13,04,2016 10:00:00'
...
def getIncrementalDate(){
    return new SimpleDateFormat('dd,MM,yyyy HH:mm:ss').parse(incrementalDateTime);
}
...
salesOrgList.each{
	...
	writetofile("Contact_Id;Customer_Id;Title;FirstName;LastName;AdditionalName;Department;PhoneNumbers;Mobilenumber;Faxnumber;Email;Function;ErpCustomerID\r",cont_filename)
	...
	query = """SELECT distinct innerSelect.pk FROM 
                ({{
                    SELECT {bu.pk} FROM { 
                                ...
                                JOIN MovexOrder AS mo ON {mo.customerNrIntershop} = {bu.uid}
                          }
	...
	b2bUnits.each{
         ...
         if(unit?.getModifiedtime()?.after(getIncrementalDate()))
         {
	    ...
	    unit?.getMembers()?.each{
	        if (it instanceof B2BCustomerModel) {
	            contact = (B2BCustomerModel) it
                if(contact?.getModifiedtime()?.after(getIncrementalDate()))
                {
        ...
        if(customer_data?.toString().length()>0){
	      customer_data << "$semicolon"
        } 
	    
	    unit?.getAddresses()?.each{
            if(it?.getModifiedtime()?.after(getIncrementalDate()))
            {
